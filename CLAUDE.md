...
**Ключ RAWG пока не задан** — поэтому в проде работает только запасной Steam, то есть
консольные игры не находятся. Владелец должен получить бесплатный ключ на rawg.io/apidocs
и добавить его в Vercel как `RAWG_API_KEY` (ввод ключей в поля мне запрещён), после чего
сделать Redeploy.
